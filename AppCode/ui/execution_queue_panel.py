...
class ExecutionQueuePanel(QWidget):
    ...
    def _init_ui(self):
        ...
        title_layout.addWidget(self.column_settings_btn)
        ...
        self.list_widget.setDragDropMode(QListWidget.InternalMove)  # 支持拖拽排序
    ...
    def _build_item_text(self, index, path):
        """根据列设置构建列表项文本"""
        info = self._script_info_map.get(path, {})
        name = info.get('name', path)

        parts = [f"{index+1}. {name}"]

        if self._column_visibility.get('path'):
            parts.append(f"  [{path}]")

        if self._column_visibility.get('category'):
    ...
    def _show_column_settings(self):
        ...
        path_action = QAction("显示路径", self)
        path_action.setCheckable(True)
        path_action.setChecked(self._column_visibility['path'])
        path_action.triggered.connect(lambda: self._toggle_column('path'))
        menu.addAction(path_action)

        category_action = QAction("显示分类", self)
        category_action.setCheckable(True)
        category_action.setChecked(self._column_visibility['category'])
        category_action.triggered.connect(lambda: self._toggle_column('category'))
    ...
    def _toggle_column(self, column_key):
        """切换列显示"""
        self._column_visibility[column_key] = not self._column_visibility[column_key]
        self._update_list()
    ...
    def _update_list(self):
        ...
        try:
            ...
            for i, path in enumerate(self._queue):
                item = QListWidgetItem(self._build_item_text(i, path))
    ...
    def _on_rows_moved(self, parent, start, end, dest, row):
        ...
        for i in range(self.list_widget.count()):
            ...
            # 更新编号显示
            item.setText(self._build_item_text(i, path))

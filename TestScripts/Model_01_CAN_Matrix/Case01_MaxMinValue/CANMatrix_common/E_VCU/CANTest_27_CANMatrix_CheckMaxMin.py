...
if __name__ == '__main__':
    ...
    InitAllCANMessage()
    STLA_CAN.DBC.SetSignal(1,'DAT_TBMU_314','MIN_ALLOW_DISCHRG_VOLTAGE',0)
    StartTime = TimeStamp()     # 获取毫秒级时间戳
    Sleep(15000)
    EndTime = TimeStamp()       # 获取毫秒级时间戳
    STLA_CAN.DBC.SetSignal(1,'DAT_TBMU_314','MIN_ALLOW_DISCHRG_VOLTAGE',1000)
    Sleep(5000)
    STLA_CAN.DBC.RecordState(AcqEnum.Stop)
    CloseAllDevice()
    CloseAllCANMessage()

    Test, dArrTimeStamp1, dArrTimeStampList1= STLA_CAN.DBC.FindValueOfSignal('DAT_TBMU_314','MIN_ALLOW_DISCHRG_VOLTAGE',0,ConditionEnum.Equal,0,0,[],[])
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('DAT_TBMU_314','MIN_ALLOW_DISCHRG_VOLTAGE',1000,ConditionEnum.Equal,0,0,[],[])
    FindReqTime = dArrTimeStamp1[0]
    FindStateTime = dArrTimeStamp2[0]

    CursorPosition = [FindReqTime, FindStateTime]
    STLA_CAN.DBC.ExportWfmFile(['DAT_TBMU_314'],
                                ['MIN_ALLOW_DISCHRG_VOLTAGE'],
                                ''
    )
    Sleep(2000)

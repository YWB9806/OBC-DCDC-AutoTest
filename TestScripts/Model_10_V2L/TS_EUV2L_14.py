...
if __name__ == '__main__':
    ...
    交流源载一体机.Set.Impedance(ACeEnable.ON,ACIMPEnum.R,[14.3,0.0,0.0])
    ...
    Test, OBCACCurrent = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('P_IacRpt_IacRms_A'),'P_IacRpt_IacRms_A', 0.0)
    ...
    TestResult24 = abs(OBCACCurrent - 16) <= 1
    ...
    print(f"P_IacRpt_IacRms_A:{OBCACCurrent},测试结果2-4:{TestResult24}")

    交流源载一体机.Set.Impedance(ACeEnable.ON,ACIMPEnum.R,[23,0.0,0.0])
    Sleep(5000)
    Test, ACDCConversionState = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    Test, OBCACVoltage = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE', 0.0)
    Test, OBCACCurrent = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('P_IacRpt_IacRms_A'),'P_IacRpt_IacRms_A', 0.0)
    TestResult31 = ACDCConversionState == 4
    Test, Measure_ACVoltage = 功率分析仪.Meas.Volt(PAVoltEnum.RMS, dealListData([0]*8))
    Measure_ACVoltage = list(Measure_ACVoltage)[0]
    TestResult32 = Measure_ACVoltage > 220
    TestResult33 = abs(Measure_ACVoltage - OBCACVoltage)/Measure_ACVoltage <= 0.03
    TestResult34 = abs(OBCACCurrent - 10) <= 1
    TestResult3 = True if TestResult31 and TestResult32 and TestResult33 and TestResult34 else False
    print(f"ACDC_CONVERSION_STATE:{ACDCConversionState},测试结果3-1:{TestResult31}")
    print(f"OBC_EVSE_MES_AC_VOLTAGE:{OBCACVoltage},功率分析仪测量的交流电压:{Measure_ACVoltage},测试结果3-2:{TestResult32},测试结果3-3:{TestResult33}")
    print(f"P_IacRpt_IacRms_A:{OBCACCurrent},测试结果3-4:{TestResult34}")

    交流源载一体机.Set.Impedance(ACeEnable.ON,ACIMPEnum.R,[46,0.0,0.0])    
    Sleep(5000)
    Test, ACDCConversionState = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    Test, OBCACVoltage = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE', 0.0)
    Test, OBCACCurrent = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('P_IacRpt_IacRms_A'),'P_IacRpt_IacRms_A', 0.0)
    TestResult41 = ACDCConversionState == 4
    Test, Measure_ACVoltage = 功率分析仪.Meas.Volt(PAVoltEnum.RMS, dealListData([0]*8))
    Measure_ACVoltage = list(Measure_ACVoltage)[0]
    TestResult42 = Measure_ACVoltage > 220
    TestResult43 = abs(Measure_ACVoltage - OBCACVoltage)/Measure_ACVoltage <= 0.03
    TestResult44 = abs(OBCACCurrent - 5) <= 1
    TestResult4 = True if TestResult41 and TestResult42 and TestResult43 and TestResult44 else False
    print(f"ACDC_CONVERSION_STATE:{ACDCConversionState},测试结果4-1:{TestResult41}")
    print(f"OBC_EVSE_MES_AC_VOLTAGE:{OBCACVoltage},功率分析仪测量的交流电压:{Measure_ACVoltage},测试结果4-2:{TestResult42},测试结果4-3:{TestResult43}")
    print(f"P_IacRpt_IacRms_A:{OBCACCurrent},测试结果4-4:{TestResult44}")
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('P_IacRpt_IacRms_A'),'P_IacRpt_IacRms_A',15,ConditionEnum.Morethan,0,0)
    Time1 = dArrTimeStamp1[10]
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('P_IacRpt_IacRms_A'),'P_IacRpt_IacRms_A',11,ConditionEnum.Lessthan,Time1,0)

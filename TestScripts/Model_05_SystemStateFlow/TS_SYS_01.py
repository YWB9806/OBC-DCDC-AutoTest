...
if __name__ == '__main__':
    ...
    MaxResistance = 1200
    Step = 10
    for resistance in range(SetCCResistance, MaxResistance + 1, Step):
        ...
        ResistanceDictSequence1[resistance] = ReadRes
        # ReadRes = 1000
    ...
    for resistance in range(SetCCResistance, MaxResistance + 1, Step):
        ...
        if IFB_CCCP_CC_Res >=10 and IFB_CCCP_CC_Res < 60:
            TestResult2Sequence.append(OBCChgrConnConf == 2)
            TestResult3Sequence.append(CCS_V2L_PLUG_Detected == 1)
        elif IFB_CCCP_CC_Res >=60 and IFB_CCCP_CC_Res < 750:
            TestResult2Sequence.append(OBCChgrConnConf == 2)
            TestResult3Sequence.append(CCS_V2L_PLUG_Detected == 0)
        elif IFB_CCCP_CC_Res >=750 and IFB_CCCP_CC_Res < 1200:
            TestResult2Sequence.append(OBCChgrConnConf == 2)
            TestResult3Sequence.append(CCS_V2L_PLUG_Detected == 1)
    ...
    for resistance in sorted(ResistanceDictSequence1.keys()):
        ...
        error_percent_str = error_percent if isinstance(error_percent, str) else f"{error_percent:.2f}%"
        result_str += f"{resistance:^11} | {unpowered_value:^15.2f} | {powered_value:^15.2f} | {error_percent_str:^9} | {tolerance_status:^8}\n"
    print(result_str)
    Sleep(5000)
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('CCS_V2L_PLUG_DETECTED'),'CCS_V2L_PLUG_DETECTED',1,ConditionEnum.Equal,StartTime,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('CCS_V2L_PLUG_DETECTED'),'CCS_V2L_PLUG_DETECTED',0,ConditionEnum.Equal,StartTime,0)
    Test, dArrTimeStamp3, dArrTimeStampList3 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('CCS_V2L_PLUG_DETECTED'),'CCS_V2L_PLUG_DETECTED',1,ConditionEnum.Equal,StartTime,0)
    Test, dArrTimeStamp4, dArrTimeStampList4 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_CHRG_CONN_CONF'),'OBC_CHRG_CONN_CONF',1,ConditionEnum.Equal,StartTime,0)
    Time1 = dArrTimeStamp1[0]
    Time2 = dArrTimeStamp2[50]
    Time3 = dArrTimeStamp3[50]
    Time4 = dArrTimeStamp4[50]
    ...
    TestResult3 = min(TestResult3Sequence)
    print(f"测试步骤1结果: {TestResult1}, 测试步骤2结果: {TestResult2}, 测试步骤3结果: {TestResult3}")
    TestResult = True if TestResult1 and TestResult2 and TestResult3 else False
    if TestResult == True:
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1.Report CCS_PlugDetected(OBC_CHRG_CONN_CONF)=PLUGGED.==>{TestResult2}\n"+
        f"2.10~60Ω,CCS_V2L_PLUG_DETECTED=1.==>{TestResult3}\n"+
        f"3.60~750Ω,CCS_V2L_PLUG_DETECTED=0.==>{TestResult3}\n"+
        f"4.750~1200Ω,CCS_V2L_PLUG_DETECTED=1.==>{TestResult3}\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

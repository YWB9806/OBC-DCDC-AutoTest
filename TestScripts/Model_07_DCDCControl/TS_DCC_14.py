...
if __name__ == '__main__':
    ...
    TestResult12 = abs(signal_voltage1 - 10.5) <= 0.2
    ...
    TestResult21 = DCDCStateBB2 == 3
    TestResult22 = abs(signal_voltage2 - 16) <= 0.2
    TestResult2 = TestResult21 and TestResult22
    print(f"DCDC状态:{DCDCStateBB2},测试结果:{TestResult21}")
    print(f"DCDC电压:{signal_voltage2},测试结果:{TestResult22}")

    STLA_CAN.DBC.SetSignal(1, get_message_for_signal('VOLT_SETPOINT_DCDC_REQ'), 'VOLT_SETPOINT_DCDC_REQ', 10.6)  
    Sleep(5000)
    Test, DCDCStateBB2 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('DCDC_STATE_BB'),'DCDC_STATE_BB', 0.0)
    Test, signal_voltage3 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('DCDC_CONV_LV_VOLT_HD'),'DCDC_CONV_LV_VOLT_HD', 0.0)
    TestResult31 = DCDCStateBB2 == 3
    TestResult32 = abs(signal_voltage3 - 10.5) <= 0.2
    TestResult3 = TestResult31 and TestResult32
    print(f"DCDC状态:{DCDCStateBB2},测试结果:{TestResult31}")
    print(f"DCDC电压:{signal_voltage3},测试结果:{TestResult32}")
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('LD_IORpt_LvVol'),'LD_IORpt_LvVol',10.4,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('LD_IORpt_LvVol'),'LD_IORpt_LvVol',15.5,ConditionEnum.Morethan,0,0)
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1. The DCDC enters BUCK mode, and output voltage is {signal_voltage1}V.\n"+
        f"2. The DCDC enters BUCK mode, and output voltage is {signal_voltage2}V.\n"+
        f"3. The DCDC enters BUCK mode, and output voltage is {signal_voltage3}V.\n"+
        f"\n"+

...
if __name__ == '__main__':
    ...
    STLA_CAN.DBC.SetSignal(1, 'V2_BSI_552', 'CPT_TEMPOREL_2', 6553.5)
    STLA_CAN.DBC.SetSignal(1, 'V2_BSI_552', 'CPT_TEMPOREL_1', 6553.5)
    Sleep(80000)

    Test, MeasureKL30Current = 低压辅源.SCPI.Query(':MEAS:CURR? CH1')
    MeasureKL30Current = list(MeasureKL30Current)
    TestResult11 = MeasureKL30Current[0] <= 0.05
    print(MeasureKL30Current[0], TestResult11)

    低压辅源.SCPI.Write(':SOUR2:VOLT 5;:SOUR2:CURR 3;:OUTP CH2,ON')
    STLA_CAN.DBC.SetSignal(1, 'DYN_E_VCU_342', 'REVEIL_PRINCIPAL', 2)
    STLA_CAN.DBC.Trigger(1, 'DAT_E_VCU_402', False)
    Sleep(15000)
    STLA_CAN.DBC.SetSignal(1, 'V2_BSI_552', 'CPT_TEMPOREL_2', 0)
    STLA_CAN.DBC.SetSignal(1, 'V2_BSI_552', 'CPT_TEMPOREL_1', 10)
    Sleep(5000)

    def CheckIsNotCodeResult(AssignmentCode):
        Index = 0
        LengthData = 30
        ResultData = []
    ...
    AssignmentCode = 89
    TestResult12 = CheckIsNotCodeResult(AssignmentCode)
    Sleep(10000)
    ...
    Test, ReferenceHoraire = STLA_CAN.DBC.GetSignal(1,'NEW_JDD_OBC_DCDC_5B1','REFERENCE_HORAIRE', 0.0)
    TestResult13 = ReferenceHoraire==0
    TestResult1 = True if TestResult11 and TestResult12 and TestResult13 else False
    print(f"测试步骤1结果: {TestResult1}, 测试步骤1测量KL30电流: {MeasureKL30Current[0]}, 测试步骤2结果: {TestResult12}, 测试步骤3结果: {TestResult13}")
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1= STLA_CAN.DBC.FindValueOfSignal('NEW_JDD_OBC_DCDC_5B1','REFERENCE_HORAIRE',0,ConditionEnum.Equal,0,0,[],[])
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('NEW_JDD_OBC_DCDC_5B1','REFERENCE_HORAIRE',429496729.3,ConditionEnum.Equal,0,0,[],[])
    ...
    STLA_CAN.DBC.ExportWfmFile(['IFB_VersionInfo','IFB_Volt','IFB_Volt','IFB_Volt','SUPV_V2_OBC_DCDC_591','IFB_CC_CP','IFB_CC_CP',
                                ...
                                ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                                 ...
                                 'ECU_ELEC_STATE_RCD','REFERENCE_HORAIRE'],
                                ''
    )
    Sleep(2000)

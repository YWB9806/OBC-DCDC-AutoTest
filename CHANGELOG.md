...
### 修复
- 🐛 **保存测试方案卡顿+消息错乱**：修复保存方案后触发加载方案提示的问题，断开信号避免连锁触发
- 🐛 **执行耗时计算错误**：修复每脚本耗时从批次开始时间计算的问题，改为使用脚本实际执行开始时间
- 🐛 **测试结果判定优化**：仅认定中文"合格"/"不合格"为最终结果，True/False/pass/fail等英文标识均改为"待判定"
- ✨ **批次汇总信息**：执行结果新增批次汇总，显示总计/合格/不合格/待判定/运行总时间/通过率
- 🐛 **对比对话框改进**：新增详情展示区，点击表格行即可查看完整输出内容
- ✨ **报告模板工作表选择**：配置映射时支持选择Excel工作簿中的具体工作表
- 🐛 **登录窗口图标**：修复登录界面左上角图标不显示的问题
...
### 修复
- 🐛 **执行输出截断（彻底修复）**：重写stdout读取机制
  - 移除每行读取创建新线程的设计缺陷（线程泄漏+数据丢失）
  - 改为专用读取线程+Queue队列，彻底解决脚本中存在sleep时输出停止的问题
- 🐛 **保存测试方案卡顿**：方案保存操作改为后台线程异步执行
- 🐛 **更新检查误报**：修复GitHub API失败时错误显示"已是最新版本"的问题
  - 区分网络错误和无更新两种状态，API失败时给出明确提示
- 🎨 **窗口标题显示**：增加默认窗口宽度至980px，确保标题文字完整显示
...
### 修复
- 🐛 **执行输出截断**：修复脚本执行输出过早停止的问题
  - 结果关键词空闲超时从5秒延长至30秒，避免正常运行的脚本被误判为已完成
  - 新增最低输出行数阈值（30行），防止早期日志中的关键词误触发
  - 新增连续2次确认机制，避免偶发性输出停顿导致误终止
- 🐛 **加载方案卡顿**：修复加载测试方案时界面卡死的问题
  - 目录扫描改为后台线程异步执行，UI保持响应
- 🐛 **暂停/继续崩溃**：移除暂停/继续功能
  - 因Windows平台进程暂停机制（psutil.suspend）存在权限问题和稳定性风险，已移除相关功能

### 新增
- ✨ **默认目录设置**：新增脚本目录持久化功能
  - 设置界面新增"脚本目录"选项卡，可添加默认加载的脚本文件夹
  - 添加的目录自动保存到配置文件，重启软件后自动加载
...
### 优化
- 🎨 **搜索功能改进**：实现实时按文件名过滤，输入即搜索，搜索时自动展开匹配节点
- 🎨 **关于对话框精简**：删除功能特性列表，保留软件名称、开发者信息（Yangwenbo, ywb9806@163.com）
- 🐛 **登录界面图标**：修复登录界面左上角图标未显示的问题（将图标文件名从中文改为英文避免编码问题）
...
### 修复
- 🐛 **登录崩溃问题**：修复了点击登录后软件直接崩溃的严重问题
  - 根因：PBKDF2密码哈希验证（100000次迭代）在主线程执行，导致UI冻结，Windows发送`KeyboardInterrupt`强制终止进程
  - 修复：密码验证放入`QThread`后台线程，UI保持响应
- 🐛 **首次登录失败**：修复了登录失败后密码框被清空，导致用户必须重新输入密码才能重试的问题
  - 修复：登录失败后保留密码输入框内容

### 新增
- ✨ **插件功能**：支持配置外部应用程序快捷启动
  - 设置界面新增"插件设置"选项卡，支持添加/删除外部程序
  - 菜单栏新增"插件"菜单，显示已配置的外部程序，点击即可启动
  - 配置保存在 `config.json` 的 `plugins.items` 中

### 优化
- 🎨 **UI布局优化**：
  - 菜单栏新增"方案"菜单，将"保存方案"和"管理方案"从左侧按钮移至菜单栏
  - 左侧目录树默认隐藏路径列，只显示脚本名称
  - 左上部分删除"分类"下拉框，将"测试方案"下拉框和"一键折叠/展开"合并为一行
  - 执行控制按钮移除Unicode图标符号，统一使用纯中文文字
  - 右侧执行队列新增"列设置"按钮，可切换显示路径和分类列
- 🐛 **模板预览修复**：修复了配置映射中模板预览区域无内容显示的问题
  - 根因：`_load_preview()` 方法已定义但从未调用
  - 修复：在初始化时调用 `_load_preview()` 加载Excel前5行数据

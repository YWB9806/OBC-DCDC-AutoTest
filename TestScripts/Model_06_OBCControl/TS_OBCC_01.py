...
if __name__ == '__main__':
    ...
    all_results = {}
    test_ranges = [("20-30V", 20, 30),("50-55V", 50, 55),("150-155V", 150, 155),("250-255V", 250, 255)]
    for name, start, end in test_ranges:
        all_results[name] = STLA_M_EncapsulationClass.TestACVoltage(start, end)

    交流源载一体机.Set.Volt(dealListData([305, 305, 305]), dealListData([0.0, 0.0, 0.0]))
    Sleep(3000)
    Test, signal_volt = STLA_CAN.DBC.GetSignal(1, get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'), 'OBC_EVSE_MES_AC_VOLTAGE', 0.0)
    Test, measure_data = 功率分析仪.Meas.Volt(PAVoltEnum.RMS, dealListData([0,0,0,0,0,0,0,0]))
    pa_volt = list(measure_data)[0]
    special_test = {'settings': [305],'measure_pa': [],'measure_signal': [],'errors': [],'is_pass': []}
    special_test['measure_pa'].append(list(measure_data)[0])
    special_test['measure_signal'].append(signal_volt)
    special_test['errors'].append(abs(pa_volt - signal_volt) / pa_volt * 100)
    special_test['is_pass'].append(abs(pa_volt - signal_volt) / pa_volt * 100 <= 3)
    all_results["305V"] = special_test
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE',20,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE',50,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp3, dArrTimeStampList3 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE',150,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp4, dArrTimeStampList4 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_EVSE_MES_AC_VOLTAGE'),'OBC_EVSE_MES_AC_VOLTAGE',250,ConditionEnum.Morethan,0,0)
    ...
    signal_list = ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                   'IFB_CCCP_CC_Res','OBC_FAULT_STATE','DCDC_FAULT_STATE','OBC_EVSE_MES_AC_VOLTAGE']
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1.The OBC input voltage is in the range of 20-305V, and the signal PItoHVConversion_ACVoltage can be reported in real time\n"+
        f"\n"+
        f"Note:AC voltage ≤100V, detection accuracy: ±3V;\n"+
        f"Note:AC voltage > 100V, detection accuracy: ±3%\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

...
if __name__ == '__main__':
    ...
    test_configs = [
        {
            "name": "DCDC_REAL_LOAD",
            "setcurrents": [0, 100, 150, 200, 250],
            "expectcurrents": [0, 100, 150, 200, 250]
        },
    ]

    all_results = {}
    for config in test_configs:
        results, report, passed = STLA_M_EncapsulationClass.TestLVDCCurrent_Buck(
            config["name"],
            config["setcurrents"],
            config["expectcurrents"]
        )
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('DCDC_REAL_LV_CURR_HD'),'DCDC_REAL_LV_CURR_HD',45,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('DCDC_REAL_LV_CURR_HD'),'DCDC_REAL_LV_CURR_HD',95,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp3, dArrTimeStampList3 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('DCDC_REAL_LV_CURR_HD'),'DCDC_REAL_LV_CURR_HD',195,ConditionEnum.Morethan,0,0)
    Test, dArrTimeStamp4, dArrTimeStampList4 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('DCDC_REAL_LV_CURR_HD'),'DCDC_REAL_LV_CURR_HD',240,ConditionEnum.Morethan,0,0)
    ...
    signal_list = ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                   'IFB_CCCP_CC_Res','OBC_FAULT_STATE','DCDC_FAULT_STATE','LD_IORpt_LvCur','REG_MODE_DCDC_BB_REQ','DCDC_STATE_BB','DCDC_REAL_LOAD_HD']
    ...
    final_report = "DCDC_REAL_LOAD测试最终汇总\n" + "="*100 + "\n"
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1-3.DCDC_REAL_LOAD_HD=DCDC_CONV_LV_VOLT_HD*DCDC_REAL_LV_CURR_HD/DCDC_REAL_ELV_MAX_POWER.\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

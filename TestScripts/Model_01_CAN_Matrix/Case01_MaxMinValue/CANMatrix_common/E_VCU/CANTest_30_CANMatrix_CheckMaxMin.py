...
if __name__ == '__main__':
    ...
    InitAllCANMessage()
    STLA_CAN.DBC.SetSignal(1,'DYN_E_VCU_342','VOLT_SETPOINT_DCDC_REQ',10.6)
    StartTime = TimeStamp()     # 获取毫秒级时间戳
    Sleep(15000)
    STLA_CAN.DBC.SetSignal(1,'DYN_E_VCU_342','VOLT_SETPOINT_DCDC_REQ',16)
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1= STLA_CAN.DBC.FindValueOfSignal('DYN_E_VCU_342','VOLT_SETPOINT_DCDC_REQ',10.6,ConditionEnum.Equal,0,0,[],[])
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('DYN_E_VCU_342','VOLT_SETPOINT_DCDC_REQ',16,ConditionEnum.Equal,0,0,[],[])
    FindReqTime = dArrTimeStamp1[10]
    FindStateTime = dArrTimeStamp2[0]

    CursorPosition = [FindReqTime, EndTime]
    STLA_CAN.DBC.ExportWfmFile(['DYN_E_VCU_342'],

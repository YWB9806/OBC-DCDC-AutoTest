...
if __name__ == '__main__':
    ...
    Sleep(5500)
    Test, OBCTemp = STLA_CAN.DBC.GetSignal(1,'DAT_OBC_DCDC_501','OBC_TEMP', 0.0)
    TestResult = OBCTemp == 0 # 0x28
    print(f"OBC_TEMP: {OBCTemp}, TestResult: {TestResult}")
    ...
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('DAT_OBC_DCDC_501','OBC_TEMP',0,ConditionEnum.Equal,StartTime,StartTime,[],[])
    OBCTemp = dArrTimeStampList2[0]
    TestResult = OBCTemp == 0 # 0x28
    print(f"OBC_TEMP: {OBCTemp}, TestResult: {TestResult}")

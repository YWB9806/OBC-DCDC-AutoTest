...
if __name__ == '__main__':
    ...
    for i in range(17):
        ...
        Test, IFB_CVN_CheckSum1 = STLA_CAN.DBC.GetSignal(1,'IFB_CVN','IFB_CVN_CheckSum1', 0.0)
        print(f"信号IFB_CVN_CheckSum1的值为: {IFB_CVN_CheckSum1}")
        信号发生器.SCPI.Write(':SOUR1:APPL:DC 1,1,3.0;:OUTP1 ON')
        Sleep(75000)
        Test, MeasureKL30Current = 低压辅源.SCPI.Query(':MEAS:CURR? CH1')
        MeasureKL30Current = list(MeasureKL30Current)
        TestResult1 = MeasureKL30Current[0] <= 0.05
        TestResultList.append(TestResult1)
        print(MeasureKL30Current[0], TestResult1)

        if IFB_CVN_CheckSum1 >= 16:
            break

    TestResult11 = min(TestResultList)
    信号发生器.SCPI.Write(':SOUR1:APPL:DC 1,1,9.5;:OUTP1 ON')
    Sleep(5000)
    电阻控制板.Modbus.SetRegister(iID=1,Reg=3072,Data='2700')
    Sleep(5000)
    ...
    TestResult12 = IFB_CVN_CheckSum0 == 0
    TestResult13 = OBCChrgConnConf == 0
    TestResult1 = True if TestResult11 and TestResult12 and TestResult13 else False
    print(f"操作步骤1结果:{TestResult11}, 操作步骤2结果: {TestResult12}, 操作步骤3结果: {TestResult13}")
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1= STLA_CAN.DBC.FindValueOfSignal('IFB_CVN','IFB_CVN_CheckSum1',16,ConditionEnum.Equal,0,0,[],[])
    StartTime1 = dArrTimeStamp1[0]
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('DAT_OBC_DCDC_421','OBC_CHRG_CONN_CONF',0,ConditionEnum.Equal,StartTime1,0,[],[])
    ...
    STLA_CAN.DBC.ExportWfmFile(['IFB_VersionInfo','IFB_Volt','IFB_Volt','IFB_Volt','SUPV_V2_OBC_DCDC_591','IFB_CC_CP','IFB_CC_CP',
                                ...
                                'DAT_OBC_DCDC_531','SUPV_V2_OBC_DCDC_591','DAT_E_VCU_402','DAT_OBC_DCDC_551','IFB_CVN','IFB_CVN','TBMU_V2_4F4',
                                'DAT_OBC_DCDC_421'],
                                ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                                 ...
                                 'HV_BATT_CH_NEED_STATE','OBC_CHRG_CONN_CONF'],
                                ''
    )
    Sleep(2000)

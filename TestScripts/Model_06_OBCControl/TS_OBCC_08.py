...
if __name__ == '__main__':
    ...
    test_configs = [
        {
            "name": "HVDC电流测试",
            "CurrentCommand": [40, 20],
            "ExpectCurrentList": [-30, -20]
        },
    ]
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-28,ConditionEnum.Lessthan,0,0)
    Time1 = dArrTimeStamp1[5]
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-19,ConditionEnum.Lessthan,Time1,0)
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1. The OBC can run normally , wait 5s , check  Output current is 30.3A. \n"+
        f"2. The OBC can run normally , wait 5s , and Output current is 20A. \n"+
        f"\n"+

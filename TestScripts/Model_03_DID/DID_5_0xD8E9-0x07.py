...
if __name__ == '__main__':
    ...
    交流源载一体机.Set.Volt(dealListData([310.0, 310.0, 310.0]),dealListData([0.0, 0.0, 0.0]))
    Sleep(5000)
    Test, AcdcConversionState = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    Test, EvseGridFault = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('EVSE_GRID_FAULT'),'EVSE_GRID_FAULT', 0.0)
    TestResult11 = AcdcConversionState == 0
    TestResult12 = EvseGridFault == 1
    TestResult1 = True if TestResult11 and TestResult12 else False
    print(f"AcdcConversionState的值为{AcdcConversionState}, 测试结果为{TestResult11}")
    print(f"EvseGridFault的值为{EvseGridFault}, 测试结果为{TestResult12}")
    ...
    Test, dArrTimeStamp0, dArrTimeStampList0= STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE',7,ConditionEnum.Equal,0,0,[],[])
    ...
    signal_list = ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                   ...
                   'ECU_ELEC_STATE_RCD','DCDC_STATE_BB','ACDC_CONVERSION_REQUEST','ACDC_CONVERSION_STATE','DATA_DIAG','EVSE_GRID_FAULT']

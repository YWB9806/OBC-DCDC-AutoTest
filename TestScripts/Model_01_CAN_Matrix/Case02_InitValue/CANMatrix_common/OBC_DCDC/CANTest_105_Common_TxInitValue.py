...
if __name__ == '__main__':
    ...
    Sleep(5500)
    Test, DCDCRealTemp = STLA_CAN.DBC.GetSignal(1,'DAT_OBC_DCDC_531','DCDC_REAL_TEMP', 0.0)
    TestResult = DCDCRealTemp == 0 
    print(f"DCDC_REAL_TEMP: {DCDCRealTemp}, TestResult: {TestResult}")
    ...
    Test, dArrTimeStamp2, dArrTimeStampList2= STLA_CAN.DBC.FindValueOfSignal('DAT_OBC_DCDC_531','DCDC_REAL_TEMP',-40,ConditionEnum.Equal,StartTime,StartTime,[],[])
    DCDCRealTemp = dArrTimeStampList2[0]
    TestResult = DCDCRealTemp == -40 
    print(f"DCDC_REAL_TEMP: {DCDCRealTemp}, TestResult: {TestResult}")
    ...
    STLA_CAN.DBC.ExportWfmFile(['IFB_VersionInfo','IFB_Volt','IFB_Volt','IFB_Volt','SUPV_V2_OBC_DCDC_591','IFB_CC_CP','IFB_CC_CP',
                                ...
                                ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                                 ...
                                 'ECU_ELEC_STATE_RCD','DCDC_REAL_TEMP'],
                                ''
    )
    Sleep(2000)

...
if __name__ == '__main__':
    ...
    信号发生器.SCPI.Write(':SOUR1:APPL:SQU 125,12,0,0;:SOUR1:FUNC:SQU:DCYC 53;:OUTP1 ON')
    高压源载一体机.Set.Basic(DCPwrModeEnum.NORM, DCPwrWorkModeEnum.NONE)
    高压源载一体机.Set.Volt(dVolt=350)
    高压源载一体机.Out.Enable(eEnable.ON)
    Sleep(5000)

    _ , TestResult2 = STLA_M_EncapsulationClass.CANMatrixForbiddenValueTest('MAX_INDI_DISCHRG_CURR', forbidden_values=[2001, 2044, 2045], normal_value=0, fault_code=132)
    ...
    Test1, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('MAX_INDI_DISCHRG_CURR'),'MAX_INDI_DISCHRG_CURR',2000,ConditionEnum.Morethan,0,0)
    StartTime1 = dArrTimeStamp1[0]
    Test2, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('IFB_V_VerInfo6or13or20'),'IFB_V_VerInfo6or13or20',132,ConditionEnum.Equal,StartTime1,0)
    EndTime1 = dArrTimeStamp2[0]
    Test3, dArrTimeStamp3, dArrTimeStampList3 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('MAX_INDI_DISCHRG_CURR'),'MAX_INDI_DISCHRG_CURR',2040,ConditionEnum.Morethan,0,0)
    StartTime2 = dArrTimeStamp3[0]
    Test4, dArrTimeStamp4, dArrTimeStampList4 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('IFB_V_VerInfo6or13or20'),'IFB_V_VerInfo6or13or20',132,ConditionEnum.Equal,StartTime2,0)
    ...
    signal_list = ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE','DCDC_FAULT_STATE','OBC_FAULT_STATE',
                   ...
                   'ECU_ELEC_STATE_RCD','DCDC_STATE_BB','ACDC_CONVERSION_REQUEST','ACDC_CONVERSION_STATE','MAX_INDI_DISCHRG_CURR'
                   ]
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        ...
        f"2. /.\n"+
        f"3. Triggering a Level 2 failure, and reporting IFB_V_VerInfo6or13or20=132, and the replacemennt is NA.\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

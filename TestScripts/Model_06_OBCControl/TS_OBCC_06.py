...
if __name__ == '__main__':
    ...
    Sleep(2000)
    Test, AcdcConversionState = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    PreCondition3 = AcdcConversionState == 2
    PreCondition = True if PreCondition1 and PreCondition2 and PreCondition3 else False

    test_configs = [
        {
            "name": "HVDC电流测试",
            "CurrentCommand": [5, 10, 20, 30],
            "ExpectCurrentList": [-5, -10, -20, -30]
        },
    ]

    all_results = {}
    for config in test_configs:
        results, report, passed = STLA_M_EncapsulationClass.TestHVDCCurrent_CurrentRegulation(
            config["CurrentCommand"],
            config["ExpectCurrentList"],
            config["name"]
        )
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-4,ConditionEnum.Lessthan,0,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-9,ConditionEnum.Lessthan,0,0)
    Test, dArrTimeStamp3, dArrTimeStampList3 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-19,ConditionEnum.Lessthan,0,0)
    Test, dArrTimeStamp4, dArrTimeStampList4 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('OBC_DCDC_HV_CURRENT'),'OBC_DCDC_HV_CURRENT',-29,ConditionEnum.Lessthan,0,0)
    Time1 = dArrTimeStamp1[5]
    Time2 = dArrTimeStamp2[5]
    Time3 = dArrTimeStamp3[5]
    Time4 = dArrTimeStamp4[5]
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f" 1.The OBC  can run normally,and  OBC output current is 5A/10A/20A/30A. \n"+
        f"\n"+
        f"Note:The output current control accuracy < ±1%\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

...
if __name__ == '__main__':
    ...
    TestResult2 = True if TestResult21 and TestResult22 and TestResult23 else False
    ...
    STLA_CAN.DBC.SetSignal(1, get_message_for_signal('ACDC_CONVERSION_REQUEST'), 'ACDC_CONVERSION_REQUEST', 0)
    Sleep(5000)
    Time3 = TimeStamp()     # 获取毫秒级时间戳
    Test, ACDCConversionState1 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    Test, BidirV2LAvail1 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('BIDIR_V2L_AVAIL'),'BIDIR_V2L_AVAIL', 0.0)
    TestResult31 = ACDCConversionState1 == 0
    TestResult32 = BidirV2LAvail1 == 0
    TestResult3 = True if TestResult31 and TestResult32 else False
    print(f"ACDC_CONVERSION_STATE:{ACDCConversionState1},测试结果3-1:{TestResult31}")
    print(f"BIDIR_V2L_AVAIL:{BidirV2LAvail1},测试结果3-2:{TestResult32}")

    STLA_M_EncapsulationClass.V2LCPVoltagePrecondition()
    STLA_CAN.DBC.SetSignal(1, get_message_for_signal('ACDC_CONVERSION_REQUEST'), 'ACDC_CONVERSION_REQUEST', 3)
    Sleep(5000)
    Time4 = TimeStamp()     # 获取毫秒级时间戳
    Test, ACDCConversionState2 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE', 0.0)
    Test, BidirV2LAvail2 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('BIDIR_V2L_AVAIL'),'BIDIR_V2L_AVAIL', 0.0)
    TestResult41 = ACDCConversionState2 == 4
    TestResult42 = BidirV2LAvail2 == 1
    TestResult4 = True if TestResult41 and TestResult42 else False
    print(f"ACDC_CONVERSION_STATE:{ACDCConversionState},测试结果4-1:{TestResult41}")
    print(f"BIDIR_V2L_AVAIL:{BidirV2LAvail},测试结果4-2:{TestResult42}")
    ...
    Test, dArrTimeStamp1, dArrTimeStampList1 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('ACDC_CONVERSION_REQUEST'),'ACDC_CONVERSION_REQUEST',0,ConditionEnum.Equal,StartTime,0)
    Test, dArrTimeStamp2, dArrTimeStampList2 = STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('ACDC_CONVERSION_STATE'),'ACDC_CONVERSION_STATE',0,ConditionEnum.Equal,StartTime,0)
    Time1 = dArrTimeStamp1[0]
    Time2 = dArrTimeStamp2[0]
    TestResult5 = abs(Time2 - Time1) < 1000
    print(f"时间差值:{abs(Time2 - Time1)},测试结果5:{TestResult5}")
    ...
    Log4NetWrapper.WriteToOutput_KeyInfo(TestResultDisplay, 
        f"\n"+
        f"1.BIDIR_V2L_AVAIL= {BidirV2LAvail1}; ACDC_CONVERSION_STATE={ACDCConversionState1},Exit the V2L working mode within {abs(Time2 - Time1)}ms after the trigger.\n"+
        f"2.The V2L mode is re-entered,BIDIR_V2L_AVAIL={BidirV2LAvail2}, ACDC_CONVERSION_STATE={ACDCConversionState2}.\n"+
        f"\n"+
        f"========================================================================="+
        f"\n"+
        f"{all_prints}"

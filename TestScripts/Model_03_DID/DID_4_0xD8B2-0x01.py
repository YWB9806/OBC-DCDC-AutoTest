...
if __name__ == '__main__':
    ...
    STLA_CAN.DBC.SetSignal(1, get_message_for_signal('REVEIL_PRINCIPAL'), 'REVEIL_PRINCIPAL', 0)  
    信号发生器.SCPI.Write(':SOUR1:APPL:DC 1,1,4.4;:OUTP1 ON')
    Sleep(5000)
    ...
    Sleep(5000)
    StartTime = TimeStamp()
    Test, IFB_CVN_CheckSum0 = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('IFB_CVN_CheckSum0'),'IFB_CVN_CheckSum0', 0.0)
    if IFB_CVN_CheckSum0 == 1:

        Test, ECUElecStateRCD = STLA_CAN.DBC.GetSignal(1,get_message_for_signal('ECU_ELEC_STATE_RCD'),'ECU_ELEC_STATE_RCD', 0.0)
        TestResult2 = ECUElecStateRCD == 1
        ...
        STLA_CAN.DBC.SetSignal(1, get_message_for_signal('DATA_DIAG_2'), 'DATA_DIAG_2', 55474)  
    ...
    # Test, dArrTimeStamp0, dArrTimeStampList0= STLA_CAN.DBC.FindValueOfSignal(get_message_for_signal('ECU_ELEC_STATE_RCD'),'ECU_ELEC_STATE_RCD',5,ConditionEnum.Equal,StartTime,0,[],[])
    # StartTime1 = dArrTimeStamp0[0]

    Test, EndTime1 = STLA_CAN.DBC.FindMsg('REP_DIAG_ON_CAN_58F',0,0,FindMsgTypeEnum.First,0)
    CursorPosition = replace_zeros([StartTime, EndTime1])
    signal_list = ['IFB_V_VerInfo6or13or20','IFB_Volt_KL30','IFB_Volt_KL15','IFB_Volt_LVCC','RCD_LINE_STATE',
                   ...
                   'IFB_CVN_CheckSum0','ECU_ELEC_STATE_RCD','DCDC_STATE_BB','ACDC_CONVERSION_REQUEST','ACDC_CONVERSION_STATE','DATA_DIAG']
